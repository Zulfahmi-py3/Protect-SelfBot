...
sw = LineClient()
sw = LineClient(authToken='Ghost TOKEN')
...
wait = {
    ...
    "comment":"Auto like by ZULFAHMI",
    "message":"Terimakasih sudah add INDO BOT ALIANSI \n kalau ada perlu bisa chat Owner Kami http://line.me/ti/p/~zulfahmi78 \n menerima jasa buat bot .. thanks 😃",
    }
...
def help():
    ...
    helpMessage = "╔═════════════════\n" +\
                  ...
                  "╠➩●➢" + key + "IBAcuaca「Nama Kota」\n" + \
                  "╠➩●➢" + key + "IBAlocation「Nama Kota」\n" + \
                  "╠➩●➢" + key + "IBAmusic「Judul Lagu」\n" + \
                  "╠➩●➢" + key + "IBAlyric「Judul Lagu」\n" + \
                  "╠➩●➢" + key + "Ytmp3:「Judul Lagu」\n" + \
                  "╠➩●➢" + key + "image: \n" + \
                  "╠➩●➢" + key + "IBAyoutube「Judul Video」\n" + \
                  ...
                  "╠➩●➢" + key + "Listprotect\n" + \
                  "╠══════════════════════\n" +\
                  "║●●●●" + key + "INDO BOT ALIANSI ●●●●\n" +\
                  "║" + key + " ●● http://line.me/ti/p/~zulfahmi78 ●● \n" +\
                  "╚══════════════════════"
    return helpMessage
                    ...
                    if text is None:
                        ...
                        if cmd == "self on":
                          ...
                          if wait["selfbot"] == True:
                            ...
                            if msg._from in admin:
                                cl.sendText(msg.to,"INDO BOT ALIANSI Creator") 
                                ma = ""
                                for i in creator:
                                    ma = cl.getContact(i)
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               ...
                               cl.sendMessage1(msg)
                               cl.sendMessage(msg.to,"「 SQUAD INDO BOT ALIANSI」")
                          ...
                          if msg._from in admin:
                            try:
                                ...
                                cl.sendMessage(msg.to, "「 SQUAD INDO BOT ALIANSI 」Grup Info\n\nNama Group : {}".format(G.name)+ "\n❧ID Group : {}".format(G.id)+ "\n❧Pembuat : {}".format(G.creator.displayName)+ "\n❧Waktu Dibuat : {}".format(str(timeCreated))+ "\n❧Jumlah Member : {}".format(str(len(G.members)))+ "\n❧Jumlah Pending : {}".format(gPending)+ "\n❧Group Qr : {}".format(gQr)+ "\n❧Group Ticket : {}".format(gTicket))
                          ...
                          if msg._from in admin:
                            ...
                            try:
                                ...
                                timeCreated = []
                                timeCreated.append(time.strftime("%d-%m-%Y [ %H:%M:%S ]", time.localtime(int(G.createdTime) / 1000)))
                                ret_ += "●➢「 SQUAD INDO BOT ALIANSI」 Grup Info\n"
                                ret_ += "\n●➢Nama Group : {}".format(G.name)
                                ret_ += "\n●➢ID Group : {}".format(G.id)
...
#===========BOT UPDATE============#
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                cl.sendMessage(msg.to,"●➢「 INDO BOT ALIANSI」\n\n"+ma+"\nTotal「%s」 Bots" %(str(len(Bots))))
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                cl.sendMessage(msg.to,"●➢「ADMIN INDO BOT ALIANSI 」\n\nSuper admin:\n"+ma+"\nAdmin:\n"+mb+"\nStaff:\n"+mc+"\nTotal「%s」「SQUAD INDO BOT ALIANSI 」 " %(str(len(owner)+len(admin)+len(staff))))
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                G.preventedJoinByTicket = True
                                kc.updateGroup(G)
                                ki.sendMessage(msg.to,"「 INDO BOT ALIANSI 1 DONE」")
                                kk.sendMessage(msg.to,"「INDO BOT ALIANSI 2 DONE」")
                                kc.sendMessage(msg.to," 「 INDO BOT ALIANSI 3 DONE」")
                                sw.sendMessage(msg.to,"「 INDO BOT ALIANSI COMPLETE .. GHOST ANTIJS STAY」")

                        elif cmd == "out":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                G = cl.getGroup(msg.to)
                                ki.sendText(msg.to, "「SQUAD INDO BOT ALIANSI 1 2 3 PAMIT DULU .. MAU CARI MAKAN!! BYE 」 "+str(G.name))
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                cl.sendMessage(msg.to, "●➢「 INDO BOT ALIANSI RESPON」\n\n  ●➢「 Get Profile」\n   %.10f\n ●➢「Get Contact」\n   %.10f\n ●➢「Get Group」\n   %.10f" % (get_profile_time/3,get_contact_time/3,get_group_time/3))

                        elif text.lower() == 'speed':
                        	if msg._from in admin:
                                 start = time.time()
                                 cl.sendMessage(to, "●➢ INDO BOT ALIANSI SPEED FULL..")
...
#===========Hiburan============#
                        ...
                        elif cmd.startswith("IBAlocation "):
                                try:
                                    sep = text.split(" ")
                                    location = text.replace(sep[0] + " ","")
                        ...
                        elif cmd.startswith("IBAyoutube "):
                                sep = text.split(" ")
...
#===========Protection============#
                        ...
                        elif 'IBApro ' in msg.text:
                           if msg._from in admin:
                              spl = msg.text.replace('IBApro ','')
                              if spl == 'on':
                                  if msg.to in protectqr:
                                       msgs = ""
                                  else:
...
#===========COMMAND SET============#
                           ...
                           if msg._from in admin:
                            ...
                            if msg._from in admin:
                               cl.sendMessage(to," 「ADMIN INDO BOT ALIANSI HADIR」")
                               ki.sendMessage(to," 「BOTS INDO BOT ALIANSI 1」")
                               kk.sendMessage(to," 「BOTS  INDO BOT ALIANSI 2」")
                               kc.sendMessage(to,"「 BOTS  INDO BOT ALIANSI 3 」")
                               sw.sendMessage(to," 「MY GHOST STAY .. SAVE YOUR GROUP」 ")
                               
                        elif cmd == "my team" or msg.text.lower == "my team":
                      	  if msg._from in admin:
                       	    cl.sendMessage(to,"╔═════════════ \n ╠➢ Open Order Bot .. /n ╠➢ 1 admin + 2 asist /n ╠➢ 1 admin + 4 asist + 1 ghost /n ╠═══════════ /n ║ ●● jika minat silahkan contact kami ●● /n ╠➢ https://line.me/ti/p/~zulfahmi78 /n ╚══════════════")
